    ...
    let next =
        match node.level {
            ...
            1 => {
                ...
                let node_key_keys =
                    Array::<Array<HL::Leaf, 2, DIMENSION>, 2, DIMENSION>::build_array(
                        #[inline(always)]
                        |index_vec| {
                            hl.get_node_key(node_key.as_ref().map_node(
                                #[inline(always)]
                                |node| node[index_vec].clone(),
                            ))
                            .leaf()
                            .unwrap()
                        },
                    );
                let final_key = Array::try_build_array(
                    #[inline(always)]
            ...
            _ if node.level - 1 == log2_step_size => {
                let node_key = hl.get_node_key(node.clone()).node().unwrap();
                let node_key_keys =
                    Array::<Array<HL::NodeId, 2, DIMENSION>, 2, DIMENSION>::build_array(
                        #[inline(always)]
                        |index_vec| {
                            hl.get_node_key(node_key.as_ref().map_node(
                                #[inline(always)]
            ...
            _ => {
                ...
                ParallelBuildArray::<HL::NodeId, HL::Error, 3, DIMENSION>::parallel_build_array(
                    ...
                    #[inline(always)]
                |index_vec3| -> Result<HL::NodeId, HL::Error> {
                        let key = Array::build_array(#[inline(always)]
                        |index_vec2| {
                            let sum_vec = index_vec3 + index_vec2;
                            let sum_div2 = sum_vec.map(#[inline(always)]
                            |v| v / 2);
                            let sum_mod2 = sum_vec.map(#[inline(always)]
                            |v| v % 2);
                            node_key_keys[sum_div2][sum_mod2].clone()
                        });
                        let temp = hl.intern_non_leaf_node(NodeAndLevel {
